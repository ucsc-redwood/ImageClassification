...
int main() {
    ...
    // Load the weight data for the second convolutional layer
    int secondWeightDataSize;
    float* second_weight_data = readDataFromFile("features_3_weight.txt", secondWeightDataSize);

    // Load the bias data for the second convolutional layer
    ...
    int second_conv_output_height = pooled_output_height; // height and width remain the same due to padding = 1 and stride = 1
    int second_conv_output_width = pooled_output_width;
    float* second_conv_output_data = new float[192 * second_conv_output_height * second_conv_output_width]; // 192 output channels

    // Call the convolution function for the second layer
    conv2d(maxpool_output_data, 64, pooled_output_height, pooled_output_width, // 64 input channels, dimensions from max pooling
           second_weight_data, 192, 64, 3, 3, // 192 output channels, 64 input channels, 3x3 kernel size
           second_bias_data, 192, 3, 1, 1, // 192 bias elements, kernel size 3, stride 1, padding 1
    ...
    // Output the result after the second max pooling layer
    std::cout << "Output after the second max pooling layer:" << std::endl;
    for (int c = 0; c < 192; ++c) {
        std::cout << "Channel " << c + 1 << ":" << std::endl;
        for (int i = 0; i < second_pooled_output_height; ++i) {
            for (int j = 0; j < second_pooled_output_width; ++j) {
                std::cout << second_maxpool_output_data[(c * second_pooled_output_height + i) * second_pooled_output_width + j] << " ";
            }
            std::cout << std::endl;
        }
        std::cout << std::endl;
