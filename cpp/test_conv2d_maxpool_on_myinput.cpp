...
// Include the maxpool2d function here
float* maxpool2d(float* input_data, int input_channels, int input_height, int input_width,
    ...
    float* output_data = new float[input_channels * output_height * output_width];
...
int main() {
    ...
    float* conv_output_data = conv2d(image_data, image_input_channels, input_height, input_width,
    ...
    float* maxpool_output_data = maxpool2d(conv_output_data, weight_output_channels, conv_output_height, conv_output_width, pool_size, pool_stride);

    // Output the result after max pooling
    std::cout << "Output after max pooling:" << std::endl;
    for (int c = 0; c < weight_output_channels; ++c) {
        std::cout << "Channel " << c + 1 << ":" << std::endl;
        for (int i = 0; i < (conv_output_height - pool_size) / pool_stride + 1; ++i) {
            for (int j = 0; j < (conv_output_width - pool_size) / pool_stride + 1; ++j) {
                std::cout << maxpool_output_data[(c * ((conv_output_height - pool_size) / pool_stride + 1) + i) * ((conv_output_width - pool_size) / pool_stride + 1) + j] << " ";
            }
            std::cout << std::endl;
        }
        std::cout << std::endl;
