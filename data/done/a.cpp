...
void Second_conv2d(const float input[4][4][64], int input_size,
            ...
            float output[32][32][192], int &output_size) {
    // Validate the kernel size, stride, and padding
...
void Second_ReLU(float output[32][32][192], int output_size) {
    for (int c = 0; c < 192; ++c) {
        for (int i = 0; i < output_size; ++i) {
            for (int j = 0; j < output_size; ++j) {
                if (output[i][j][c] < 0) {
...
void Second_maxpool2d(const float input[32][32][192], int input_size,
               int pool_size, int stride, float output[16][16][192], int &output_size) {
    // Correct calculation of output_size
    output_size = (input_size - pool_size) / stride + 1;
...
void Third_conv2d(const float input[16][16][192], int input_size,
                  const float kernel[3][3][192][384],
                  const float bias[384], int stride, int padding,
                  float output[8][8][384], int &output_size) {
    // Validate the kernel size, stride, and padding
...
void Third_ReLU(float output[8][8][384], int output_size) {
    for (int c = 0; c < 384; ++c) {
        for (int i = 0; i < output_size; ++i) {
            for (int j = 0; j < output_size; ++j) {
                if (output[i][j][c] < 0) {
...
void Fourth_conv2d(const float input[8][8][384], int input_size,
                   const float kernel[3][3][384][256],
                   const float bias[256], int stride, int padding,
                   float output[8][8][256], int &output_size) {
    // Validate the kernel size, stride, and padding
    if (3 > input_size || 3 <= 0 || stride <= 0 || padding < 0) {
        std::cout << "Invalid kernel size, stride, or padding." << std::endl;
        return;
...
/*
void Third_maxpool2d(const float input[8][8][384], int input_size,
               int pool_size, int stride, float output[4][4][384], int &output_size) {
    // Correct calculation of output_size
    output_size = (input_size - pool_size) / stride + 1;
    ...
    cout << "Third_maxpool2d Input: " << input_size << "x" << input_size << "x" << 384 << " " << "Third_maxpool2d Output: " << output_size << "x" << output_size << "x" << 384 << endl;
}
*/
...
//--------------------LAYER ONE CONV INPUTS------------------------//
    ...
    ifstream weight_file1("features_0_weight.txt");
    ...
    // Read bias from file
    ifstream bias_file1("features_0_bias.txt");
    float bias1[64];
    for (int i = 0; i < 64; ++i) {
        bias_file1 >> bias1[i];
    }
...
// Read kernel weights from file
    ifstream weight_file2("features_3_weight.txt");
    ...
    // Read bias from file
    ifstream bias_file2("features_3_bias.txt");
    float bias2[192];
...
//--------------------LAYER THREE CONV INPUTS---------------------//
    // Read kernel weights from file
    ifstream weight_file3("features_6_weight.txt");
    ...
    // Read bias from file
    ifstream bias_file3("features_6_bias.txt");
    float bias3[384];
...
//--------------------LAYER FOUR CONV INPUTS---------------------//
    ...
    //float kernel4[3][3][384][256]; // Note the order of dimensions
    ...
    for (int out_channel = 0; out_channel < 256; ++out_channel) {
        for (int in_channel = 0; in_channel < 384; ++in_channel) {
            for (int i = 0; i < 3; ++i) {
                for (int j = 0; j < 3; ++j) {
			count++;
			//cout<<"i: " << i << " " << "j: " << j << " " << "in_channels: " << in_channel << " " << "out_channels: " << out_channel << endl;  
			//weight_file4 >> kernel4[i][j][in_channel][out_channel]; // Note the order of indices
...
//--------------------Output Arrays------------------------------//
    ...
    // Declare output arrays
    float conv_output2[32][32][192];
    int conv_output_size2;
    // Declare maxpool output arrays
    float maxpool_output2[16][16][192];
    int maxpool_output_size2;

    // Declare output arrays
    float conv_output3[8][8][384];
    int conv_output_size3;
    // Declare maxpool output arrays
    float maxpool_output3[4][4][384];
    int maxpool_output_size3;

    // Declare output arrays
    float conv_output4[8][8][256];
    int conv_output_size4;

    // Call conv2d function
    cout << "First Conv2 running now" << endl;
    ...
    Second_conv2d(maxpool_output1, 32, kernel2, bias2, 2, 5, conv_output2, conv_output_size2);
    ...
    Third_conv2d(maxpool_output2, 16, kernel3, bias3, 1, 3, conv_output3, conv_output_size3);
    Third_ReLU(conv_output3, conv_output_size3);
    //Fourth_conv2d(conv_output3, 8, kernel4, bias4, 1, 3, conv_output4, conv_output_size4);
    //Third_maxpool2d(conv_output3, conv_output_size3, 2, 2, maxpool_output3, maxpool_output_size3);
}
